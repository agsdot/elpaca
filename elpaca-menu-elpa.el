...
(defun elpaca-menu-elpa--recipes (elpa)
  ...
  (let ((name (alist-get 'name elpa)))
    (message "Downloading %s recipes" name)
    (with-current-buffer (url-retrieve-synchronously (alist-get 'packages-url elpa))
...
(defun elpaca-menu-elpa--metadata (elpa)
  ...
  (or (alist-get 'metadata-cache elpa)
      (when (libxml-available-p)
        (require 'dom)
        (with-current-buffer (url-retrieve-synchronously (alist-get 'metadata-url elpa))
...
(defun elpaca-menu-elpa--index (elpa)
  ...
  (cl-loop
   ...
   for item =
   (when-let
       (((not (plist-get props :core))) ;;@TODO: Can we do this w/out cloning Emacs?
        ((if releasep (plist-get props :release-branch) t))
        (name (symbol-name id))
        (url (if-let ((declared (plist-get props :url))
        ...
        (recipe `( :package ,name :repo ,url :local-repo ,name
                   ,@(or (and (eq url remote)
                              `(:branch ,(concat branch-prefix (when releasep "-release") "/" name)))
  ...
  (cond ((eq request 'index)
         (or (alist-get 'cache elpa)
             (prog1
               ...
               (when recurse (message "%s menu updated." (alist-get 'name elpa))))))
        ((eq request 'update)
